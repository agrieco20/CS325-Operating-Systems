 ...
 * @Date: 9/24/2023
 *
 * File Description: This file constructs a Priority Queue, which is used to sort the jobs in the queue based on which has the shortest estimated completion time and prioritizes them so that they will be scheduled to run on the SAC-SimOS first. This particular Priority Queue takes the form of a Min Heap.
 * Note: This Priority Queue structure is used exclusively by Anthony Grieco's "com.sos.os.CPU_Scheduler", which intends to minimize the average waiting time of all processes within the queue even at the potential cost of starvation.
 *
...
public class PriorityQueue {
    ...
    static KeyPair keyNode; //Used to represent the current/newest node added to the "Heap". The Key of the keyNode will be compared to the Key of its parent node to see if they should be swapped (based on which element has the lower Key)
    static KeyPair parent; //Used to represent the Parent node of the keyNode (so that their Keys can be compared and their placements swapped if the keyNode's key is lower)
    static KeyPair root; //Indicates which node is at the top of the "Heap" (has the lowest Key value and will be the node that is Popped and whose value will subsequently be returned). It is also stored at the first index of the "Heap" Arraylist
    static int tempNodeKey; //Key: Acts as a temporary variable so that the "keyNode" and "parent" nodes can be swapped if "keyNode" has a lower Key than its "parent" does.
    static SimProcessInfo tempNodeProcess; //Value: Acts as a temporary variable so that the "keyNode" and "parent" nodes can be swapped if "keyNode" has a lower Key than its "parent" does.
    ...
    static KeyPair smallest; //Indicates the node in the parent-child nodes currently being looked at that has the smallest Key value

    int nodeKey; //Used to temporarily store the Key (estimated completion time) of a job being moved within the Priority Queue
    SimProcessInfo nodeProcess; //Used to temporarily store a job itself that is being moved within the Priority Queue
